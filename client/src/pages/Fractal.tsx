...
export default function Fractal() {
  ...
  const activeAxis = active
    ? dialecticalAxes.find(
        ax => ax.agentA === active.code || ax.agentB === active.code
      )
    : null;
  const partnerCode = activeAxis
    ? activeAxis.agentA === active?.code
      ? activeAxis.agentB
      : activeAxis.agentA
    : null;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden">
      {/* Background */}
      <div
        className="fixed inset-0 z-0 opacity-8"
        style={{
      ...
      <div className="absolute inset-x-0 top-0 p-4 md:p-6 flex flex-wrap items-center justify-between gap-3 z-30">
        <div>
          <Link
            href="/"
            className="text-xl md:text-2xl font-semibold tracking-tight tech-text hover:text-[oklch(0.7_0.15_200)] transition-colors"
          >
            DIA • Canon Fractal
          </Link>
          <p className="text-xs text-muted-foreground mt-1">
            Visualisation dynamique — positions en perpétuelle évolution
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-sm text-xs tech-text transition-all ${
              showAxes
                ? "bg-[oklch(0.65_0.2_330)] text-black"
                : "bg-card/80 backdrop-blur-sm border border-border"
            }`}
          >
            Axes
          ...
          <Link
            href="/canon"
            className="px-3 py-1.5 rounded-sm text-xs tech-text bg-card/80 backdrop-blur-sm border border-border hover:border-[oklch(0.7_0.15_200)] transition-all"
          >
            Canon →
          </Link>
        </div>
      ...
      <div className="w-full h-screen flex items-center justify-center p-4">
        ...
        >
          ...
          {/* Strate rings */}
          {showStrates &&
            (Object.entries(STRATE_RADII) as [Strate, number][]).map(
              ([key, r]) => (
                <g key={key}>
                  <circle
                    cx={CX}
          ...
          {/* Dialectical axes */}
          {showAxes &&
            dialecticalAxes.map(axis => {
              const nodeA = nodes.find(n => n.code === axis.agentA);
              const nodeB = nodes.find(n => n.code === axis.agentB);
              if (!nodeA || !nodeB) return null;
              const isActive =
                active &&
                (active.code === axis.agentA || active.code === axis.agentB);
              const pulse =
                Math.sin(time * 3 + dialecticalAxes.indexOf(axis)) * 0.3 + 0.5;
              return (
                <line
                  key={axis.id}
                  x1={nodeA.x}
              ...
              <g key={n.code}>
                ...
                {imgUrl ? (
                  <image
                    ...
                    onClick={() =>
                      setPinned(p => (p && p.code === n.code ? null : n))
                    }
        ...
        {active && (
          <motion.div
            ...
            style={{
              ...
              borderRadius:
                getAgentTheme(active.code)?.shapes.borderRadius || "12px",
            }}
          >
            {/* Header */}
            <div className="flex items-start justify-between gap-3 mb-3">
              <div>
                <div className="text-xs text-muted-foreground tech-text">
                  {active.code}
                </div>
                <div className="text-xl font-semibold tech-text mt-1">
                  {active.name}{" "}
                  <span className="text-muted-foreground greek-text text-lg">
                    {active.greek}
                  </span>
                </div>
              </div>
              {agentImages[active.code] ? (
                <div
                  ...
                  style={{
                    ...
                    boxShadow: `0 0 16px ${active.color}60`,
                  }}
                >
                  <img
                    src={agentImages[active.code]}
                    alt={active.name}
            ...
            <div className="flex items-center gap-2 mb-3">
              <span
                className="px-2 py-0.5 rounded text-xs tech-text"
                style={{
            ...
            {activeAxis && (
              <div className="mb-3 p-3 rounded-sm bg-background/50 border border-border">
                <div className="text-xs text-muted-foreground mb-1 tech-text">
                  AXE DIALECTIQUE
                </div>
            ...
            <div className="mt-3 flex items-center justify-between gap-3">
              <Link
                href={`/agent/${active.code.toLowerCase()}`}
                className="text-xs hover:underline tech-text"
                style={{
                  color:
                    getAgentTheme(active.code)?.palette.accent || "#00d4ff",
                }}
              >
                Voir détails →
              </Link>
      ...
      <div className="absolute left-4 bottom-4 text-xs text-muted-foreground space-y-1 z-30 max-w-xs">
        <div className="flex items-center gap-3 flex-wrap">
          {(
            Object.entries(strates) as [Strate, (typeof strates)[Strate]][]
          ).map(([key, s]) => (
        ...
        <div>
          Survol: aperçu • Clic: épingler • Les agents orbitent en temps réel
        </div>
      </div>
    </div>
  );
